...
(defn enliven-string [s]
  ...
  ;(URL. "http://boards.mpora.com/forums/looking-ahead-f6.html")))
  ...
  (defn match-page [topic-regex root-link page-number]
    (let [page-url (str "/forums/" root-link "p" page-number ".html") 
          [match] (re-seq topic-regex  (apply str (line-seq (clojure.java.io/reader(get-forum-stream page-url)))))]
    ...
    ([topic-regex root-link last-page] 
      (if-let [[max-pages-match] (re-seq #"p(\d+).html" last-page)] 
        (get-pages-containing 
          topic-regex root-link last-page [] (pmap inc (range (Integer/parseInt (last max-pages-match)))))))
  ...
  (defn find-topics-on []
    (if-let [{:keys [title root-link last-page]} (forum-thread-starting (enliven-string(get-forum-stream looking-ahead-forum)))]
